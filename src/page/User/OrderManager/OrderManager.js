...
function ManagerOrder() {
    const { loading, getOrderByUserDetailFunction, userOrderDetail } = useContext(myContext);
    const user = JSON?.parse(localStorage.getItem('user') || [])

    useEffect(() => {
        getOrderByUserDetailFunction(user?.uid);
            ...
            {loading ?
                <div><Loader></Loader>
                    ...
                    {userOrderDetail?.length <= 0 ?
                        <div className="px-8 flex items-center justify-center flex-col gap-2 ">
